...
//
    List<Facility> getFacilitiesByJobs(Facility facility);
    ...
    @Query(value = "SELECT SUM(HOURS_OF_WORKING) FROM REPORT LEFT JOIN FACILITY ON FACILITY.ID=REPORT.FACILITY_ID WHERE FACILITY.ID =?1 AND " +
            "REPORT.REPORT_DAY BETWEEN date_trunc('month', current_date) + make_interval(0,?2-2) and date_trunc('month', current_date) + make_interval(0,?2-1)", nativeQuery = true)
